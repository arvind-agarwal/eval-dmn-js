...
const { decisionTable, dateTime } = require('../src/index');

function readFile(filename) {
  return fs.readFileSync(filename, { encoding: 'UTF-8' });
}

describe('Parse and evaluate decision tables', function() {

  it.only('Parse DRG', async function() {
    const decisions = await decisionTable.parseDmnXml(readFile("./test/data/test.dmn"));
  ...
  it('Parse decision table', async function() {
    const decisions = await decisionTable.parseDmnXml(readFile("./test/data/test-type-error.dmn"));
    expect(decisions).not.to.be.undefined;
  });
